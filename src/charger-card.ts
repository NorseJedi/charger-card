...
export class ChargerCard extends LitElement {
        ...
        if (data === undefined || data === null) {
            ...
            for (const [statekey, stateval] of Object.entries(data)) {
                ...
                if (stateval !== null && stateval != undefined) {
                    for (const [key, val] of Object.entries((stateval as {
                        [index: string | number]: cardServiceEntity[]
                    }[]))) {
    ...
    getCardCheckData(val) {
        ...
        const usenum: number = +data['useval'];    //cast to number
        if (!isNaN(usenum)) {
        ...
        if (data['conditional_entity'] !== undefined && data['conditional_entity'] !== null) {
            data['hide'] = false;
            let cond_attr;
            const cond_state = this.getEntityState(data['conditional_entity']);
            data['hide'] = cond_state !== null && (cond_state == 'off' || cond_state == 'false') ? true : data['hide'];
    ...
    createServiceData(service, isRequest, service_data, event) {
        ...
        let service_target = {};
        for (const [key, val] of Object.entries((service_data as { [index: string | number]: string | Object }))) {
            if (key == 'target') {
                service_target = val;
            } else {
                ...
                if (typeof val == 'string') {
                    ...
                    const valObj = { ...val };
                    for (const [k2, v2] of Object.entries(valObj as { [index: string | number]: string })) {
    ...
    // Determine if charger mode is 'smart' or 'normal'
    getChargingMode() {
        let chargingmode = 'normal';
        const carddata = this.getCardData(this.getConfig('details.smartcharging'));
        if (carddata !== null && carddata !== undefined && typeof carddata === 'object' && carddata['entity'] !== null) {
            switch (this.config.brand) {
                case 'zaptec':
                    let enabled_features = 0;
                    if (carddata['attribute'] == null) {
                        enabled_features = parseInt(carddata['entity']['attributes']['enabled_features']);
                    } else {
                        enabled_features = parseInt(carddata['entity']['attributes'][carddata['attribute']]);
                    }
                    if ((enabled_features & 8) == 8) {
                        chargingmode = 'smart';
                    }
                    break;
                default:
                    if (carddata['entity'].state == 'on') {
                        chargingmode = 'smart';
                    }
            }
        }
    ...
    renderLeds(state) {
        ...
        const chargingmode = this.getChargingMode();
        const compactview = this.compactView ? '-compact' : '';

        let imageled: string;
        let ledCssClass = `charger led${compactview}`;
        switch (this.config.brand) {
            case 'zaptec':
                imageled = cconst.LEDIMAGES_ZAPTEC[chargingmode][state] || cconst.LEDIMAGES_ZAPTEC[chargingmode]['DEFAULT'];
                ledCssClass += ' zaptec';
                break;
            default:
                imageled = cconst.LEDIMAGES[chargingmode][state] || cconst.LEDIMAGES[chargingmode]['DEFAULT'];
        }
        return html`<img class="${ledCssClass}" style="visibility:${visible}" src="${imageled}"
                         @click="${() => this.handleMore(carddatas != null ? carddatas['entity'] : null)}"
                         ?more-info="true"/> `;
    }
    ...
    renderStatus() {
        ...
        return html`
            <div class="status${compactview}"
                 @click="${() => this.handleMore(carddata_status != null ? carddata_status['entity'] : null)}"
                 ?more-info="true">
                <span class="status-text${compactview}" alt=${status}>${status}${statusunit}</span>

                <div class="status-detail-text${compactview}" alt=${substatus}
                     @click="${() => this.handleMore(carddata_substatus != null ? carddata_substatus['entity'] : null)}"
                     ?more-info="true">
            ...
            return html`
                ...
                >
                    <div class="tooltip">
                        ...
                        <span
                            class="tooltiptext">${this.loc(carddata.text, 'common', this.brand)} ${carddata.unit_showontext ? '(' + carddata.unit + ')' : ''}</span>
                    </div>
                </div>
            `;
                ...
                return html`
                    ...
                    >
                        <div class='tooltip'>
                            ...
                            <span
                                class='tooltiptext${tooltip}'>${this.loc(carddata.text, 'common', this.brand)} ${carddata.unit_showontext ? '(' + carddata.unit + ')' : ''}</span>
                        </div>
                    </div>
                `
            }) : ''}`;
    ...
    renderCompact() {
        ...
        switch (this.config.brand) {
            case 'zaptec':
                state = this.entity !== undefined ? this.entity['attributes']['charger_operation_mode'] : null;
                break;
            default:
                state = this.entity !== undefined ? this.entity['state'] : null;
        }
